...
namespace EntityTools.Core
{
    public partial class EntityToolsMainPanel : /* UserControl //*/ Astral.Forms.BasePanel
    {
        ...
        private void btnTest_Click(object sender, EventArgs e)
        {
            ...
            string fullFileName = AStar.AStarDiagnostic.SaveLog();
            if (!string.IsNullOrEmpty(fullFileName)
                && MessageBox.Show(this, $"Would you like to open {Path.GetFileName(fullFileName)}?", $"Open {Path.GetFileName(fullFileName)}?", MessageBoxButtons.YesNo) == DialogResult.Yes)
