...
namespace EntityTools.Tools.ItemFilter
{
    ...
    /// </summary>
    public partial class IndexedBags<TFilterEntry> : KeyedCollection<uint, SlotCache<TFilterEntry>>, IEnumerable<SlotCache<TFilterEntry>>
    {
        protected override uint GetKeyForItem(SlotCache<TFilterEntry> slotCache)
        {
            return slotCache.FilterGroup.GroupIndex;
        }

        public SlotCache<TFilterEntry> this[ItemFilterGroup<TFilterEntry> grp]
        {
            get
            {
                ...
                if (base.Contains(grp.GroupIndex))
                    return base[grp.GroupIndex].AsReadOnly();
        ...
        public string Description()
        {
...

            {
                ...
                if (base.Count > 0)
                {
                    ...
                    {
                        if(base.Contains(filterGroup.GroupIndex))
                        {
                            var cachedSlots = base[filterGroup.GroupIndex];
