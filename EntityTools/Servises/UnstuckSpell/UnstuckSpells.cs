...
namespace EntityTools.Services
{
    public static class UnstuckSpells// : Astral.Logic.Classes.FSM.State
    {
...
#if DEBUG_SPELLSTUCKMONITOR
            Logger.WriteLine(Logger.LogType.Debug, $"{nameof(UnstuckSpells)}[{Thread.CurrentThread.ManagedThreadId.ToString("X")}]::Run() completed");
#endif
        }
...
#if DEBUG_SPELLSTUCKMONITOR
                    Logger.WriteLine(Logger.LogType.Debug, $"{nameof(UnstuckSpells)}[{Thread.CurrentThread.ManagedThreadId.ToString("X")}]::{nameof(NeedToRun)} = {result}");
...
#if DEBUG_SPELLSTUCKMONITOR
            Logger.WriteLine(Logger.LogType.Debug, $"{nameof(UnstuckSpells)}::DisableSpells_AurasCheck()");
            Logger.WriteLine(Logger.LogType.Debug, $"{nameof(UnstuckSpells)}: Character class is '{player.Character.Class.Category}'");
#endif
                ...
                case CharClassCategory.DevotedCleric:
                    {
                        ...
                        if (searchChanneldivinity)
                            Logger.WriteLine(Logger.LogType.Debug, $"{nameof(UnstuckSpells)}: SpecialClassPower[{Cleric_Channeldivinity}] not detected");
                        if (searchArbiterMechanic)
                            Logger.WriteLine(Logger.LogType.Debug, $"{nameof(UnstuckSpells)}: Aura '{Cleric_Arbiter_Mechanic}' not detected");
#endif
                ...
                case CharClassCategory.OathboundPaladin:
                    {
                        ...
                        //Если все Mods перебрали, а флаги не сброшены, значит соответствующие им умения неактивны
                        if (searchSanctuary)
                            Logger.WriteLine(Logger.LogType.Debug, $"{nameof(UnstuckSpells)}: Aura '{Paladin_Shift}' not detected");
                        if (searchDivinechampion)
                            Logger.WriteLine(Logger.LogType.Debug, $"{nameof(UnstuckSpells)}: Aura '{Paladin_Justicar_Mechanic}' not detected");
#endif
...
#if DEBUG_SPELLSTUCKMONITOR
                            else Logger.WriteLine(Logger.LogType.Debug, $"{nameof(UnstuckSpells)}: SpecialClassPower[{Paladin_Oathkeeper_Mechanic}] not active");
#endif
                        }
                        break;
                    }
        ...
        private static void DisableSpellsDelegate(object sender, Engine.EngineArgs e)
        {
            ...
            {
#if DEBUG_SPELLSTUCKMONITOR
                Logger.WriteLine(Logger.LogType.Debug, $"{nameof(UnstuckSpells)}[{Thread.CurrentThread.ManagedThreadId.ToString("X")}]: LeaveState '{e.state.DisplayName}' ... check UnstuckSpells");
#endif
                if (NeedToRun)
                {
                    var player = EntityManager.LocalPlayer;
                    ...
                    switch (player.Character.Class.Category)
                    {
                        case CharClassCategory.DevotedCleric:
                            {
#if DEBUG_SPELLSTUCKMONITOR
                                Logger.WriteLine(Logger.LogType.Debug, $"{nameof(UnstuckSpells)}[{Thread.CurrentThread.ManagedThreadId.ToString("X")}]: Character class is '{player.Character.Class.Category}'");
#endif
                                // Отключение скила 'Channeldivinity'
                                GameCommands.Execute("specialClassPower 0");
                                Logger.WriteLine($"{nameof(UnstuckSpells)}[{Thread.CurrentThread.ManagedThreadId.ToString("X")}]: '{player.Character.Class.Category}' deactivate SpecialClassPower[{Cleric_Channeldivinity}]");
...
#if DEBUG_SPELLSTUCKMONITOR
                else Logger.WriteLine(Logger.LogType.Debug, $"{nameof(UnstuckSpells)}[{Thread.CurrentThread.ManagedThreadId.ToString("X")}]: Character is in Combat ... skip");
#endif
            }
            //#if DEBUG_SPELLSTUCKMONITOR
            //            else Logger.WriteLine(Logger.LogType.Debug, $"{nameof(UnstuckSpells)}[{Thread.CurrentThread.ManagedThreadId.ToString("X")}]: LeaveState '{e.state.DisplayName}' ... skip");
            //#endif
        }
#endif
        #endregion
