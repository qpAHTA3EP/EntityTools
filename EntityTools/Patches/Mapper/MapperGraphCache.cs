...
namespace EntityTools.Patches.Mapper
{
    public class MapperGraphCache
    {
        ...
        /// <summary>
        /// Расстояние от cacheInitialPos в пределах которого кэшируются узлы графа
        /// </summary>
        public double CacheRadius
        {
            get => cacheRadius;
            set
            {
                if (value > 100)
                    cacheRadius = value;
                else cacheRadius = 100;
            }
        }
        private double cacheRadius = 150;
        ...
        public bool NeedRegenCache
        {
            get
            {
                return cacheTimeout.IsTimedOut && EntityManager.LocalPlayer.Location.Distance3D(cacheInitialPos) > cacheRadius * 0.75d;
            }
        }
        ...
        public bool RegenCache(Vector3 newIniPos = null)
        {
            ...
            if (newIniPos.IsValid)
                cacheInitialPos = newIniPos.Clone();
            else cacheInitialPos = EntityManager.LocalPlayer.Location.Clone();
        ...
        private bool NeedCache(Node node)
        {
            ...
            return dx * Math.Sign(dx) < cacheRadius
                && dy * Math.Sign(dy) < cacheRadius
                && dz * Math.Sign(dz) < cacheRadius;
        }
        ...
        Timeout cacheTimeout = new Timeout(0);
    }
}
