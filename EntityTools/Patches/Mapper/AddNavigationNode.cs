...
{
    internal class AddNavigationNode
    {
        public AddNavigationNode(float X, float Y, float Z, Graph Graph)
        {
        ...
        public AddNavigationNode(Node node, Graph graph, bool uniDirection = false/*, bool showLogs = false*/)
        {
            this.newNode = node;
            this.Graph = graph;
        ...
        public void Work1()
        {
            ...
            lock (obj)
            {
                ...
                stopwatch.Start();
#endif
                double maxDistance = Math.Max(25, EntityTools.PluginSettings.Mapper.WaipointDistance*1.5);
                double maxZDifference = EntityTools.PluginSettings.Mapper.MaxElevationDifference;
                try
                {
                    if (!this.Graph.Nodes.Contains(this.newNode))
                    {
                        int newNodeInd = this.Graph.Nodes.Add(this.newNode);
                        for (int nodeInd = 0; nodeInd < this.Graph.Nodes.Count; nodeInd++)
                        {
                            Node currentNode = (Node)this.Graph.Nodes[nodeInd];
                            //if (currentNode.X != this.node.X || currentNode.Y != this.node.Y || currentNode.Z != this.node.Z)
                            if (nodeInd != newNodeInd)
                            {
                                /*Vector3 vector = new Vector3((float)this.node.X, (float)this.node.Y, (float)this.node.Z);
                                Vector3 from = new Vector3((float)node.X, (float)node.Y, (float)node.Z);

                                double distance = vector.Distance3D(from);*/

                                double distance = Distance(currentNode, newNode);

                                double zDifference = currentNode.Z - this.newNode.Z;
                                    ...
                                    && zDifference <= maxZDifference)
                                {
                                    try
                                    {
                                        if (this.uniDirection)
                                        {
                                            this.Graph.AddArc(currentNode, this.newNode, 0f);
                                        }
                                        else
                                        {
                                            this.Graph.Add2Arcs(this.newNode, currentNode, 0f);
                                        }
                                        //if (this.showLogs)
                                        //{
                ...
                {
                    stopwatch.Stop();
                    Logger.WriteLine(Logger.LogType.Debug, $"AddNavigationNode:: Time is {stopwatch.ElapsedMilliseconds} ms ({stopwatch.ElapsedTicks})");
                }
#endif
            }
        }
        ...
        public void Work()
        {
            ...
            lock (obj)
            {
                ...
                double maxDistance = Math.Max(25, minDistance * 1.25);
                double maxZDifference = EntityTools.PluginSettings.Mapper.MaxElevationDifference;
