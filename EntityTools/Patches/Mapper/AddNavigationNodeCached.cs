...
{
    internal class AddNavigationNodeChached
    {
#if PROFILING && DEBUG
...
#endif
        public AddNavigationNodeChached(Vector3 pos, MapperGraphCache Graph)
        {
            newNode = new Node(pos.X, pos.Y, pos.Z);
            this.Graph = Graph;
            //new Thread(new ThreadStart(this.Work)).Start();
            Task.Factory.StartNew(Work);
        }

        public AddNavigationNodeChached(Node node, MapperGraphCache graph, bool uniDirection = false/*, bool showLogs = false*/)
        {
            this.newNode = node;
            this.Graph = graph;
